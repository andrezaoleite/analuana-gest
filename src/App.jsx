...
const PC=18, PL=2.80, PCO=2.50; // preços base
...
const temAcesso = (perfil, modulo) => {
  const mapa = {
    Administrador: ["dashboard","financeiro","producao","manejo","gadocorte","lancamentos","usuarios"],
    Gerente:       ["dashboard","producao","manejo","gadocorte","lancamentos"],
    Financeiro:    ["dashboard","financeiro","lancamentos"],
...
const PASTAGENS_INIT = [
  { id:1, nome:"Pasto Norte",    area:45, capacidade:30, atual:28, tipo:"Leiteiro" },
  { id:2, nome:"Pasto Sul",      area:32, capacidade:22, atual:18, tipo:"Misto"    },
  { id:3, nome:"Pasto Leste",    area:38, capacidade:22, atual:13, tipo:"Corte"    },
  { id:4, nome:"Piquete 1",      area: 8, capacidade:10, atual: 8, tipo:"Leiteiro" },
  { id:5, nome:"Piquete 2",      area: 6, capacidade: 8, atual: 2, tipo:"Corte"    },
  { id:6, nome:"Confinamento A", area: 4, capacidade:10, atual: 5, tipo:"Corte"    },
  { id:7, nome:"Confinamento B", area: 4, capacidade:10, atual: 5, tipo:"Corte"    },
];
...
function Campo({ label, value, onChange, type="text", required=false, options=null, style={} }) {
...
// ── DASHBOARD ─────────────────────────────────────────────
function DashboardView({ funcionarios, producao, despesas, receitas }) {
      ...
      {aba==="geral" && (
        <div>
          <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:18 }}>
            ...
            <KpiCard label="Funcionários"  value={funcionarios.filter(f=>f.ativo).length+" ativos"} color="#457b9d" icon="👥" trend={0}/>
          </div>
...
function ManejoView({ animaisLeiteiro, setAnimaisLeiteiro, animaisCorte, setAnimaisCorte, vacinas, setVacinas, pastagens }) {
  ...
  return (
    <div>
      ...
      <TabBar tabs={[{id:"leiteiro",label:"🐄 Gado Leiteiro"},{id:"corte",label:"🐂 Gado de Corte"},{id:"vacinas",label:"💉 Agenda Sanitária"},{id:"pastagens",label:"🌿 Pastagens"}]} active={tab} onChange={setTab}/>
      ...
      {tab==="pastagens" && (
        <div style={{ display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:14 }}>
          {pastagens.map((p,i)=>{
            ...
            const status = p.atual===0?"Descanso":ocup>=0.9?"Superlotado":ocup>=0.5?"Ocupado":"Subutilizado";
            const cor = p.atual===0?"#457b9d":ocup>=0.9?"#e76f51":ocup>=0.5?"#2d6a4f":"#d4a017";
            const tipoCor = p.tipo==="Corte"?"#457b9d":p.tipo==="Leiteiro"?"#2d6a4f":"#d4a017";
            return (
              <Card key={i}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
                  <div>
                    ...
                    <div style={{ display:"flex",gap:8,marginTop:3 }}>
                      ...
                      <span style={{ padding:"1px 8px",borderRadius:8,fontSize:11,fontWeight:600,background:tipoCor+"22",color:tipoCor }}>{p.tipo}</span>
                    </div>
                  </div>
                ...
                <div style={{ marginTop:12 }}>
                  <div style={{ display:"flex",justifyContent:"space-between",fontSize:12,color:"#6b7280",marginBottom:4 }}>
                    <span>{p.atual}/{p.capacidade} cabeças</span>
                ...
                <div style={{ marginTop:8,fontSize:11,color:"#9ca3af" }}>Cap: {p.capacidade} UA · {(p.capacidade/p.area).toFixed(1)} UA/ha</div>
              </Card>
            );
          })}
        </div>
...
// ── LOGIN ─────────────────────────────────────────────────
function LoginView({ onLogin, usuarios }) {
...
export default function App() {
  ...
  const [pastagens]                 = useState(PASTAGENS_INIT);

  if(!logado) return <LoginView onLogin={u=>{ setLogado(u); setMenu("dashboard"); }} usuarios={usuarios}/>;
    ...
    <div style={{ display:"flex",height:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f0f4f1",fontSize:14 }}>
      <div style={{ width:210,background:"#1b4332",color:"white",display:"flex",flexDirection:"column",flexShrink:0 }}>
        ...
        <nav style={{ padding:"8px 0",flex:1 }}>
          {MENU_ITEMS.map(it=>(
            <button key={it.id} onClick={()=>setMenu(it.id)} style={{ display:"flex",alignItems:"center",gap:9,width:"100%",padding:"11px 18px",border:"none",cursor:"pointer",background:menu===it.id?"#2d6a4f":"transparent",color:menu===it.id?"#d8f3dc":"#b7e4c7",fontSize:13,fontWeight:menu===it.id?700:400,textAlign:"left",borderLeft:`3px solid ${menu===it.id?"#52b788":"transparent"}` }}>
        ...
        <div style={{ padding:"12px 18px",borderTop:"1px solid #2d6a4f" }}>
          ...
          <div style={{ fontSize:9,color:"#52b788",marginTop:6 }}>v2.0 · Abr/2025</div>
        </div>
      </div>

      <div style={{ flex:1,overflow:"auto",padding:22 }}>
        {menu==="dashboard"   && <DashboardView funcionarios={funcionarios} producao={producao} despesas={despesas} receitas={receitas}/>}
