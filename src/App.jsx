...
const BotaoSecundario=({children,onClick})=><button onClick={onClick} style={{padding:"9px 16px",background:"#f3f4f6",color:"#374151",border:"1px solid #e5e7eb",borderRadius:8,cursor:"pointer",fontSize:13}}>{children}</button>;
const BotaoPerigo=({children,onClick})=><button onClick={onClick} style={{padding:"7px 14px",background:"none",color:"#dc2626",border:"1px solid #fca5a5",borderRadius:6,cursor:"pointer",fontSize:12,fontWeight:600}}>{children}</button>;
const BotaoEditar=({onClick})=><button onClick={onClick} style={{padding:"7px 12px",background:"none",color:"#2d6a4f",border:"1px solid #b7e4c7",borderRadius:6,cursor:"pointer",fontSize:12,fontWeight:600}}>✏️ Editar</button>;
...
function ModalDemarcacao({pastagem,config,mapsLoaded,onClose,onSalvar}){
  const [lat,setLat]=useState(String(pastagem?.lat||config?.lat||-14.86));
  const [lng,setLng]=useState(String(pastagem?.lng||config?.lng||-39.26));
  const mapRef=useRef(null);
  const markerRef=useRef(null);

  useEffect(()=>{
    if(!mapsLoaded||!mapRef.current||!config?.mapsApiKey) return;
    const map=new window.google.maps.Map(mapRef.current,{
  ...
  return(
    <Modal title={`📍 Demarcar no Mapa — ${pastagem?.nome}`} onClose={onClose} largura={700}>
      {!config?.mapsApiKey?(
        <div style={{padding:16,background:"#fffbeb",borderRadius:8,color:"#b45309",fontSize:13,marginBottom:16}}>Configure a chave do Maps nas Configurações do Sistema primeiro.</div>
      ):(
        <div ref={mapRef} style={{width:"100%",height:350,borderRadius:8,marginBottom:12}}/>
      )}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
        <Campo label="Latitude"  value={lat} onChange={setLat}  type="number"/>
        <Campo label="Longitude" value={lng} onChange={setLng} type="number"/>
      </div>
      <div style={{fontSize:11,color:"#6b7280",marginBottom:16}}>Clique no mapa ou arraste o marcador para posicionar. Você também pode inserir as coordenadas manualmente.</div>
      <div style={{display:"flex",justifyContent:"flex-end",gap:10}}>
        <BotaoSecundario onClick={onClose}>Cancelar</BotaoSecundario>
        <BotaoP onClick={()=>onSalvar(lat,lng)}>💾 Salvar Localização</BotaoP>
      </div>
    </Modal>
  );
}
